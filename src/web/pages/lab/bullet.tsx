...
function editBody(editor: Editor, bullet: NodeEntry<Element>): void {
  ...
  const [body] = Editor.nodes(editor, {
    at: bulletPath,
    match: (n, p) =>
      p.length === bulletPath.length + 1 && !Editor.isEditor(n) && Element.isElement(n) && n.type === 'bullet-body',
  })
...
const BulletBody = ({ attributes, children, element }: RenderElementProps): JSX.Element => {
  ...
  editor.insertBreak = () => {
    ...
    if (selection) {
      ...
      const [bulletHead] = Editor.nodes(editor, {
        match: n => !Editor.isEditor(n) && Element.isElement(n) && n.type === 'bullet-head',
      })
      if (bulletHead) {
        ...
        const [firstIndentBullet] = Editor.nodes(editor, {
          at: parentPath,
          match: (n, p) => p.length === parentPath.length + 1 && Element.isElement(n) && n.type === 'bullet',
        })
      ...
      const [bulletBody] = Editor.nodes(editor, {
        match: n => !Editor.isEditor(n) && Element.isElement(n) && n.type === 'bullet-body',
      })
      if (bulletBody) {
...
const BulletEditor = (): JSX.Element => {
  ...
  const editor = useMemo(() => withAutoComplete(withBullet(withHistory(withReact(createEditor())))), [])
  ...
  const onSelected = useCallback(
    (event: React.KeyboardEvent | React.MouseEvent, selectedTerm: string) => {
      if (search) {
        ...
        if (search.trigger === '[[') {
          insertSuggestion(editor, selectedTerm.replace(']]', ''))
          Transforms.move(editor, { distance: 2 })
        } else {
          insertSuggestion(editor, `${selectedTerm} `)
        }
      }
      setSearch(null)
    },
  ...
  function onKeyDownForBullet(event: React.KeyboardEvent) {
    ...
    if (selection && Range.isCollapsed(selection)) {
      ...
      if (bulletHead) {
        ...
        if (event.key === 'Enter' && Editor.string(editor, headPath).length === 0) {
