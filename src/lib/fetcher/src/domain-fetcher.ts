...
type DomainFetchFunction = (url: string, domain?: string) => Promise<FetchResult>
...
export const youtube: DomainFetchFunction = async function (url) {
  ...
  if (p) {
    ...
    return {
      domain,
      // TODO: [Bug]可能會有redirect, short-url, mobile-url
      resolvedUrl: url,
...
export const general: DomainFetchFunction = async function (url, domain) {
  if (domain === undefined) {
    throw new Error()
  }
  return {
