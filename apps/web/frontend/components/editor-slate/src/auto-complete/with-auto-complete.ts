    ...
    if (selection) {
      if (Range.isCollapsed(selection)) {
        ...
        if (defaultCompletes[text]) {
          ...
          const tempSelection = {
            anchor: { ...anchor, offset: anchor.offset - term.length + 1 },
            focus,
          }
          ...
          if (tempStr === term) {
            ...
            const lastBatch =
              editor.history.undos[editor.history.undos.length - 1]
            if (
              lastBatch[lastBatch.length - 1] &&
              lastBatch[lastBatch.length - 2]
            ) {
        ...
        if (defaultSelectionCompletes[text]) {
          ...
          Transforms.insertText(editor, endComplete, {
            at: {
              path: selectionFocus.path,
              offset: selectionFocus.offset + endComplete.length,
            },
          })
