...
import { ElementLic, ElementLi, ElementUl } from '../interfaces'
import { isLc, isLi, isUl } from '../utils'
  ...
  const insertPath = Path.next(Path.parent(entry[1])),
    ul: ElementUl = {
      ...
      children: [
        {
          type: 'li',
          children: [{ type: 'lic', uid: nanoid(), children: [{ text: '' }] }],
        },
      ],
    }

  Transforms.insertNodes<ElementUl>(editor, ul, { at: insertPath })
...
export function insertNextLi(
  ...
  const [, path] = entry
  Transforms.insertNodes<ElementLi>(
    editor,
    {
      type: 'li',
      children: [{ type: 'lic', uid: nanoid(), children: [{ text: '' }] }],
...
function insertPrevLi(editor: Editor, entry: NodeEntry<ElementLi>) {
  const [, path] = entry
  Transforms.insertNodes<ElementLi>(
    editor,
    {
...
function insertNextIndentLi(editor: Editor, entry: NodeEntry<ElementLi>) {
  const [node, path] = entry
  if (node.children[1] === undefined) {
    // 沒有子ul，創一個
    Transforms.insertNodes<ElementUl>(
      editor,
      {
        type: 'ul',
        children: [
          {
            type: 'li',
            children: [
              { type: 'lic', uid: nanoid(), children: [{ text: '' }] },
            ],
          },
        ],
      },
 ...
 * Indent li node
 */
export function indent(editor: Editor, entry: NodeEntry<ElementLi>): void {
  const [, path] = entry
  ...
  Transforms.setNodes<ElementLic>(
    editor,
    { change: 'change-parent' },
    { at: lcPath(nextLiPath) },
  )
}
 ...
 * Unindent li node
 */
export function unindent(editor: Editor, entry: NodeEntry<ElementLi>): void {
  const [, path] = entry
  ...
  Transforms.setNodes<ElementLc>(
    editor,
    { change: 'change-parent' },
    { at: lcPath(grandparentPath) },
  )
}
...
export const onKeyDown = (event: React.KeyboardEvent, editor: Editor): void => {
  ...
  // if (event.key === 'Shift') {
  //   const lc = Editor.above<ElementLc>(editor, { match: n => isLc(n) })
  //   console.log(lc)
  //   if (lc) {
  //     Transforms.setNodes(
  //       editor,
  ...
  if (selection && Range.isCollapsed(selection)) {
    if (!['Tab', 'Enter'].includes(event.key)) {
      return // 非tab、enter，略過
    }

    const li = Editor.above<ElementLi>(editor, { match: n => isLi(n) })
...
export const withList = (editor: Editor): Editor => {
  ...
  editor.deleteBackward = (...args) => {
    ...
    // TODO: 還未考慮一次刪除整段的情況
    if (selection && Range.isCollapsed(selection)) {
      const liEntry = Editor.above<ElementLi>(editor, { match: n => isLi(n) })
      ...
      if (liEntry) {
        ...
        if (Editor.isStart(editor, point, lcPath(liPath))) {
          ...
          const prevLiEntry = Editor.previous<ElementLi>(editor, { at: liPath })
          if (prevLiEntry) {
    ...
    if (selection && Range.isCollapsed(selection)) {
      ...
      if (liEntry) {
        ...
        if (Editor.isEnd(editor, point, lcPath(liPath))) {
          ...
          if (ul) {
            ...
            if (nextChildLi && nextChildLi.children[1] === undefined) {
              ...
              Transforms.unwrapNodes(editor, {
                ...
                match: (n, p) =>
                  Element.isElement(n) && Path.isChild(p, nextChildLiLcPath),
              })
          ...
          const nextLiEntry = Editor.next<ElementLi>(editor, { at: liPath })
          if (nextLiEntry) {
            ...
            if (nextLi.children[1]) {
              ...
              Transforms.unwrapNodes(editor, {
                at: nextLiLcPath,
                match: (n, p) =>
                  Element.isElement(n) && Path.isChild(p, nextLiLcPath),
              })
  ...
  //   const { selection } = editor
  //   if (selection) {
  //     const lc = Editor.above<ElementLc>(editor, {
  //       match: n => isLc(n),
  //     })
  //     // lc freeze，不動作
  //     if (lc && lc[0].freeze) {
