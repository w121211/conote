...
it('before, index=0', () => {
  ...
  `)
  expect(getBlock('c4').str).toMatchInlineSnapshot(`"helloworld"`)
...
it('before, index=0', () => {
  blockRepo.updateChain(
    blockSplitChainOp(getBlock('c4'), 'x', 'helloworld', 5, 'before'),
  )
  expect(() =>
    validateChildrenUids(blocksStore.getValue().entities),
  ).not.toThrow()
  ...
  `)
  expect(getBlock('c4').str).toMatchInlineSnapshot(`"world"`)
  expect(getBlock('x').str).toMatchInlineSnapshot(`"hello"`)
})
