...
export function nextBlock(block: Block, selection?: true): Block | null {
  ...
  const { open, docSymbol, childrenUids } = block
  if ((open || docSymbol) && childrenUids.length > 0) {
...
function nextSiblingRecusively(block: Block): Block | null {
  ...
  if (sib) {
    return sib
  } else if ((parent && parent.docSymbol) || block.docSymbol) {
    return null
  } else if (parent) {
...
export function prevBlock(block: Block, parent: Block): Block | null {
  ...
  if (block.order === 0 && parent.docSymbol === undefined) {
    prevBlock = parent
...
/**
 * Return the root block of the given block
 */
export function rootBlock(blockUid: string): Block {
  let block = getBlock(blockUid),
    i = 0

  while (block.parentUid !== null) {
    if (i > 1000) throw new Error('While loop reach the limit')

    block = getBlock(block.parentUid)
    i++
  }
  return block
}
