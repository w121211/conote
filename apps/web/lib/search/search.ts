...
const getSearchAuthorService = (): SearchServiceClass<Author> => {
  if (process.env.NODE_ENV === 'production') {
    return new SearchServiceClass<Author>(
      { keys: ['name'] },
      AuthorModel.getAll,
    )
  } else {
    ...
    if (!newGlobalThis[searchAuthorServicePropertyName]) {
      ...
      newGlobalThis[searchAuthorServicePropertyName] =
        new SearchServiceClass<Author>({ keys: ['name'] }, AuthorModel.getAll)
    }
...
const getSearchDiscussService = (): SearchServiceClass<Discuss> => {
  if (process.env.NODE_ENV === 'production') {
    return new SearchServiceClass<Discuss>(
      { keys: ['title'] },
      discussModel.getAll,
    )
  } else {
    ...
    if (!newGlobalThis[searchDiscussServicePropertyName]) {
      ...
      newGlobalThis[searchDiscussServicePropertyName] =
        new SearchServiceClass<Discuss>(
          { keys: ['title'] },
          discussModel.getAll,
        )
    }
