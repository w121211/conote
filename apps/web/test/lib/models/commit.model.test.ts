...
it('throws if from-doc is empty but head doc exist, content is the same', async () => {
  ...
  await expect(
    ...
    `"FROM_DOC_NOTE_HEAD::#99-from_doc_not_head, CONTENT_NOT_CHANGE::#99-from_doc_not_head"`,
  )
})
...
it('commitNoteDrafts() create sym, note if from-doc not exist', async () => {
  await testHelper.createNoteDrafts(prisma, [
    mockNoteDrafts[0],
    {
      ...mockNoteDrafts[1],
