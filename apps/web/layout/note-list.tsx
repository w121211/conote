  ...
  return (
    <div className="p-3 border border-gray-200 rounded text-sm shadow-sm">
      ...
      {data.map(({ symbol, title, updatedAt }, i) => {
        ...
        return (
          <div key={i} className="group py-3 border-b border-gray-200 last:border-none hover:cursor-pointer ">
            <div className="">
              <h5 className="flex-grow flex-shrink mb-1 truncate min-w-0 text-blue-500 g font-normal">
                {styledSymbol}
              </h5>
              <div className=" text-gray-400 text-xs">{moment(updatedAt).subtract(10, 'days').calendar()}</div>
            </div>
